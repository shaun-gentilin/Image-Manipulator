...
public abstract class AbstractFilterManip implements IManipulation {
  ...
  /**
   * Constructor for a Filter Manipulator.
   *
   * @param kernel       2D Array of kernels
   ...
   * a result of applying the kernel matrix (provided by the class) on the given pixel. This is
   * applied by putting the middle of the kernel on the specified pixel, and lining the rest of the
   * values up with the surrounding pixels.  Each pixel value is then multiplied by its
   ...
   * @param channel - an int representing the color channel that is currently being blurred. 0
   *                represents red, 1 represents green, 2 represents blue (corresponding to how they
   *                are represented in the pixel array of image).
